...
# Ask user for cleaned file
CLEANED_PATH = input("Enter the full path to the cleaned CSV file: ").strip()
...
# Ask user for path to hops template file
HOPS_PATH = input("Enter the full path to the hops CSV template: ").strip()

if not os.path.isfile(HOPS_PATH):
    print("❌ File not found:", HOPS_PATH)
    exit()

# Ask user where to save the output
OUTPUT_PATH = input("Enter the path to save the HOP-created CSV (e.g., C:\\path\\HOP-created.csv): ").strip()
...
print(f"✅ HOP-created file saved to: {OUTPUT_PATH}")
